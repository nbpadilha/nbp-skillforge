...
export function run({ root = process.cwd(), mode = "build", dryRun = false } = {}) {
  ...
  if (!existsSync(recipesAbs)) return { ok: false, errors: [{ kind: "config", msg: `no recipes directory: ${cfg.recipes} — run \`npx nbp-forge init\` to scaffold a forge project` }], drift: 0, orphans: 0 };
  ...
  for (const name of names) {
    ...
    if (mode === "check") {
      if (raw === null) { drift++; errors.push({ kind: "drift", skill: name, msg: `drift: ${cfg.out}/${name}.md is missing (run \`npx nbp-forge build\`)` }); continue; }
