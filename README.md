...
```
It's a **thin shim** that delegates to the versioned [`scripts/hooks/pre-commit`](scripts/hooks/pre-commit) bundled with the package — so the logic is reviewed in git and never drifts from what runs, whether you're in a clone or using nbp-forge as a dependency. The hook blocks a commit that (a) stages an env file (`.env`, `.env.local`, `*.env` — templates like `*.example` are allowed), (b) adds a token-shaped string (`ghp_…`, `sk-…`, `npm_…`, AWS keys, private-key headers), (c) **force-adds a git-ignored file** (`git add -f` of anything your `.gitignore` covers — stops private/internal files leaking into a public repo), or (d) leaves a generated file out of sync with its recipe. The drift-gate runs `forge:check` if your `package.json` defines it, else `npx nbp-forge check` when the repo root is a forge project. Respects `core.hooksPath`. Bypass (discouraged) with `git commit --no-verify`.
